...
<form id="form1" name="form1" method="post" action="">
  搜索: 图书代码：<input name="tushudaima" type="text" id="tushudaima" size="15" /> 
  图书名称：<input name="tushumingcheng" type="text" id="tushumingcheng" size="15" /> 
  图书分类：<input name="tushufenlei" type="text" id="tushufenlei" size="15" /> 
  图书作者：<input name="tushuzuozhe" type="text" id="tushuzuozhe" size="15" /> 是否归还：
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF" style="border-collapse:collapse">  
  <tr>
    ...
    <td width="120" align="center" bgcolor="#CCFFFF">借书时间</td>
    <td width="70" align="center" bgcolor="#CCFFFF">操作</td>
  </tr>
  <?php 
    $sql="select * from jieyuejilu where jieyueren='".$_SESSION['username']."'";
...
?></td>
	...
	<?php
		...
		{
			if (mysql_result($query,$i,"shifouguihuan")=="是" )
			{}
			else
			{
	?>
	<a href="xj.php?id=<?php echo mysql_result($query,$i,"id")?>">续借</a> <a href="hs.php?id=<?php echo mysql_result($query,$i,"id")?>&tushudaima=<?php echo mysql_result($query,$i,"tushudaima")?>&jieyongshuliang=<?php echo mysql_result($query,$i,"jieyongshuliang")?>" onClick="return confirm('确定要执行此操作？')">还书</a> 
	<?php
			}
			}
		else
